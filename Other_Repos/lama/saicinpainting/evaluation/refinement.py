...
def _infer(
    ...
    # print("shape of mask is ",mask.size())
    # print("shape of pred is ",pred.size())
...
def refine_predict( #TODO 
    ...
    if gpu_ids == ["cpu"]:
        resblocks_per_gpu = n_resnet_blocks
        devices = ["cpu"]
    else:
    ...
    for ids, (image, mask) in enumerate(zip(ls_images, ls_masks)):
        ...
        # print("Size of mask is ",mask.size())
        # input()
        mask[mask >= 1e-8] = 1.0
        mask[mask < 1e-8] = 0.0
